...
def calculate_gene_probabilities(
    ...
    Args:
        ...
        gene_def: GeneDefinition for dominance-based sorting (recommended)
    ...
    for allele1, allele2 in combinations:
        # Sort alleles by dominance for consistency
        if gene_def:
            genotype = gene_def.sort_alleles([allele1, allele2])
        else:
...
def calculate_all_genotype_combinations(
    ...
    for gene_name in registry.get_all_gene_names():
        ...
        gene_probs = calculate_gene_probabilities(p1_alleles, p2_alleles, gene_def)
